...
func largestDivisibleSubset(nums []int) []int {
	...
	ret := make([]int, maxSize)
	ret[maxSize-1] = nums[maxPos]
	maxSize--
	for i := maxPos - 1; i >= 0 && maxSize > 0; i-- {
		if maxSize == dp[i] && ret[maxSize]%nums[i] == 0 {
			maxSize--
			ret[maxSize] = nums[i]
		}
	}
